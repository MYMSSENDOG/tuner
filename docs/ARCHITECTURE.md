...
├── invariants/      벽: 튜닝으로는 깨질 수 없는 명제만 (아래 "불변식 벽")
├── fixtures/audio/  Iowa MIS 실악기 샘플 + .ref.json + 노이즈/간섭 변형
└── fixtures/notes/  악기별 크로마틱 단음 뱅크 + bank.json(창 단위 피치)
```

### 불변식 벽 (`tests/invariants/`)

나머지 스위트는 실측값을 게이트로 쓴다 — 파라미터를 재튜닝하면 정당하게
같이 움직인다. 이 디렉터리는 반대다. **임계값·파라미터·픽스처와 무관하게
참인 명제**만 들어가고, 여기가 깨졌다면 값이 드리프트한 게 아니라 무언가가
실제로 잘못된 것이다. 그래서 **여기 있는 수치는 완화하지 않는다** — 실패를
숫자로 무마하는 대신 코드를 고치거나, 규칙 자체가 바뀌었다면 근거 문서
(ARCHITECTURE / note-latch-tuning / pitch-pipeline)를 먼저 고친다.

| 파일 | 명제 | 정답의 출처 |
|---|---|---|
| `test_layering.py` | 의존은 아래로만, 순환 없음, 프레임워크는 자기 층에 | 이 문서의 "의존 규칙" |
| `test_metamorphic.py` | 음량·부호·DC·블록 크기·앞 무음·A4 를 바꿔도 표시는 그대로 | YIN 정규화와 링버퍼의 정의 |
| `test_display_contract.py` | 이름·숫자·Hz 가 서로 모순되지 않는다, 스무딩은 검출 밖의 값을 만들지 않는다 | ±50 제품 규칙 + 산수 |
| `test_arithmetic_truth.py` | 음이름 격자 **밖** 랜덤 주파수도 ±2¢ (씨드 고정) | 합성한 주파수 그 자체 |

각 파일에는 규칙이 실제로 무언가를 잡는지 확인하는 **검정력 테스트**가
같이 있다 (`..._would_catch_...`) — 아무것도 읽지 않는 검사기는 모든 것을
통과시키기 때문이다.
