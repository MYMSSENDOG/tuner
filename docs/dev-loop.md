...
- 재현 판정에 **프라이밍 갭 보정**이 필요했다. 라이브 캡처는 엔진 링이 이미
  찬 상태에서 시작하고 재생은 링을 채우며 시작하므로 `frame_size/hop`
  (4096/256 = 15)프레임이 밀린다. 세션이든 링이든 **항상 스트림 중간에서
  시작**하니 이건 예외가 아니라 정상 경로였다 — 36초 실연주 리포트에서
  "84.9% 다름"이 나왔지만 지표는 동일(195→195, 58→58)해서 잡혔다.
  `best_shift` 로 정렬을 실측하고, 재생이 스무더·래치를 빈 채 출발하는 몫
  (첫 버퍼 10프레임 + 끝 1프레임)은 `substantive()` 가 판정에서 뺀다.
