...
그와 별개로 `tests/invariants/` 는 **튜닝으로는 깨질 수 없는 명제**만 모은
벽이다 — 레이어 규칙, 같은 소리를 다르게 넣어도 같은 표시, 이름·숫자·Hz 의
자기 일관성, 음이름 격자 밖 주파수의 정확도. 여기 있는 수치는 완화하지
않는다 (`docs/ARCHITECTURE.md` "불변식 벽").
